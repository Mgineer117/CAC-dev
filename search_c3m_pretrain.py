...
def train():
    ...
    if "lbd" in config:
        args.lbd = config.lbd
    # Ablation: include the C1/C2 (Bbot-projected) conditions in the CMG warm-start.
    if "c3m_pretrain_c1c2" in config:
        args.c3m_pretrain_c1c2 = bool(config.c3m_pretrain_c1c2)

    # --- fixed CMG configuration (no longer searched) ---
    # Bounded eigenvalue-sigmoid CMG with a SIREN backbone, depth-2 / width-256,
    # fixed contraction-metric bounds and learning rates. The CMG is always
    # warm-started (SD-LQR pretraining) and then frozen for policy synthesis.
    ...
    if search_args.sweep_id is None:
        ...
        sweep_config = {
            ...
            "parameters": {
                ...
                # Ablation: minimize the C1/C2 conditions during CMG warm-start.
                "c3m_pretrain_c1c2": {
                    "values": [True, False]
                },
